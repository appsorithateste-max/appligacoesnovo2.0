...
const App = {
  ...
  async renameList(oldName) {
    ...
    try {
      ...
      this.toast('Não foi possível renomear agora. Tente novamente com conexão.');
    }
  },
  ...
  async deleteListPrompt(name) {
    ...
    try {
      ...
      this.toast('Não foi possível excluir agora. Tente novamente com conexão.');
    }
  },
  ...
  async loadDialer() {
    ...
    if (this.state.dialerListName !== listName) { this.state.skippedIds = []; this.state.dialerListName = listName; }
    ...
    try {
      ...
      if (!contact) { card.innerHTML = '<p class="label">Lista concluída! 🎉</p>'; return; }
  ...
  skipContact() {
    if (this.state.currentContact) this.state.skippedIds.push(this.state.currentContact.id);
    this.loadDialer();
  },
